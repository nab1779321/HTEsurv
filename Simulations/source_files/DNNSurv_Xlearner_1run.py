...
name_of_train_data='train1.RData'
...
reg0.fit(data0.drop(['Treatment',"Time","Event"],axis=1), d0.reshape(-1))
...
reg1.fit(data1.drop(['Treatment',"Time","Event"],axis=1), d1.reshape(-1))
...
rf = ensemble.RandomForestClassifier(n_estimators=1000,
                                     min_samples_leaf=5,
                                     max_features=1/3)
rf.fit(data.drop(['Treatment',"Time","Event"],axis=1),data['Treatment'])
...
np.savetxt(fname=output_name, delimiter=",", X=predtrue)
